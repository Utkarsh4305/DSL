...
class ProcrustesAlignment:
    ...
    def _compute_alignment_quality(self, source_points: np.ndarray,
        ...
        source_distances = cdist(source_points, source_points, metric='euclidean')
        target_distances = cdist(target_points, target_points, metric='euclidean')
        aligned_distances = cdist(aligned_source, target_points, metric='euclidean')

        # Distance correlation (how well alignment preserves relative distances)
        dist_preservation = np.corrcoef(
            source_distances.flatten(),
            aligned_distances.flatten()
        )[0, 1]
