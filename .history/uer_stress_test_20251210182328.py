...
class UERStressTester:
    ...
    def run_all_tests(self) -> Dict[str, List]:
        ...
        # Skip compiler tests due to version incompatibility
        print("3️⃣ Skipping Compiler Tests (version mismatch)")
    ...
    def _test_dimension_violations(self, validator: UERValidator):
        ...
        dim_tests = [
            (np.random.randn(512).astype(np.float32), 'under-dimension (512 vs 768)'),
            (np.random.randn(1024).astype(np.float32), 'over-dimension (1024 vs 768)'),
            (np.random.randn(769).astype(np.float32), 'off-by-one over'),
            (np.random.randn(767).astype(np.float32), 'off-by-one under'),
            (np.random.randn(768, 2).astype(np.float32), '2D embedding (batch in wrong place)'),
        ]
    ...
    def _test_normalization_edge_cases(self, validator: UERValidator):
        ...
        norm_tests = [
            ...
            (np.random.randn(768).astype(np.float32), 'random unnormalized'),
            (np.random.randn(768).astype(np.float32) / np.linalg.norm(np.random.randn(768).astype(np.float32)) * 10, 'highly amplified'),
        ]
