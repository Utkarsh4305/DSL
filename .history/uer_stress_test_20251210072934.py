...
class UERStressTester:
    ...
    def _test_quantization_issues(self, validator: UERValidator):
        ...
        quant_tests = [
            ...
            (np.full(768, -129, dtype=np.int8), 'int8 underflow'),
            (np.full(768, 128, dtype=np.int8), 'int8 overflow'),
            (np.random.randint(0, 255, 768, dtype=np.uint8), 'uint8 range ok'),
            (np.full(768, -1, dtype=np.uint8), 'uint8 underflow'),
            (np.full(768, 256, dtype=np.uint8), 'uint8 overflow'),
        ]
