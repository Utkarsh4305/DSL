...
class ProcrustesAlignment:
    ...
    def _compute_knn(self, query_points: np.ndarray, search_points: np.ndarray,
                     k: int) -> List[List[int]]:
        """
        Compute k-nearest neighbors for each query point using efficient numpy operations.
        ...
        query_norms = np.sum(query_points**2, axis=1, keepdims=True)
        search_norms = np.sum(search_points**2, axis=1, keepdims=True)
        cross_terms = query_points @ search_points.T

        distances = np.sqrt(query_norms - 2 * cross_terms + search_norms.T)

        # Set diagonal to infinity to exclude self-comparison
        # Only if query and search points are the same array
        if query_points is search_points:
        ...
        for i in range(len(query_points)):
            ...
            # Exclude self if present (when query and search are same)
            if query_points is search_points and i < len(neighbors) and neighbors[0] == i:
